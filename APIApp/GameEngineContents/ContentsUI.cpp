...
void ContentsUI::Start()
{
	//Timer
	{
		...
		TimerSets.SetImage("Number.bmp", { 60, 64 }, static_cast<int>(MarioRenderOrder::UI), RGB(255, 0, 255));
	...
	//Total Score
	{
		...
		ScoreSets.SetImage("Number.bmp", { 60, 64 }, static_cast<int>(MarioRenderOrder::UI), RGB(255, 0, 255));
		ScoreSets.SetValue(Player::TotalScore);
		ScoreSets.SetAlign(Align::Right);

		ScoreSets.SetRenderPos({ GetPos().x -500, GetPos().y });
		ScoreSets.SetCameraEffect(false);
	}
	//Number Of Coin
	{
		NumCoinSets.SetOwner(this);
		NumCoinSets.SetImage("Number.bmp", { 60, 64 }, static_cast<int>(MarioRenderOrder::UI), RGB(255, 0, 255));
		NumCoinSets.SetValue(Player::NumOfCoin);
		NumCoinSets.SetAlign(Align::Right);

		NumCoinSets.SetRenderPos({ GetPos().x - 300, GetPos().y });
		NumCoinSets.SetCameraEffect(false);
	}
	//World Level
	{
		WorldLevelSets.SetOwner(this);
		WorldLevelSets.SetImage("Number.bmp", { 60, 64 }, static_cast<int>(MarioRenderOrder::UI), RGB(255, 0, 255));
		WorldLevelSets.SetValue(Player::WorldLevel);
		WorldLevelSets.SetAlign(Align::Right);

		WorldLevelSets.SetRenderPos({ GetPos().x - 100, GetPos().y });
